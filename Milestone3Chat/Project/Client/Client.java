...
public enum Client {
    ...
    private void sendMute(String clientToMute) throws IOException//kh465 11/26/24
    {
        ...
        long mutedClientId = knownClients.entrySet().stream()
        .filter(id -> id.getValue().getClientName().equals(clientToMute)).map(Map.Entry::getKey).findFirst().orElse(null);
        LoggerUtil.INSTANCE.info(String.format("Captured username and ID: %s %s", clientToMute, mutedClientId));
        mute.setRecClientId(mutedClientId);
