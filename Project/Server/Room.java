...
public class Room implements AutoCloseable{
    ...
    protected synchronized void sendMessage(ServerThread sender, String message) {
        ...
        clientsInRoom.values().removeIf(client -> {
            ...
            if (sender == null || client.isMuted(sender.getClientName())) //kh465 12/4/24
            {
                return false;
            }
    ...
    protected void handleMute(ServerThread sender, String clientToMute) //kh465 11/26/24
    {
        clientsInRoom.forEach((id, client) -> {
            if(client.getClientName().equals(clientToMute))
            {
                if (sender.isMuted(clientToMute))
                    return;
                sender.addToMuteList(clientToMute);
                client.sendMessage(String.format("<b>%s</b> has muted you!", sender.getClientName()));
            }
        });
    }

    protected void handleUnmute(ServerThread sender, String clientToUnmute) //kh465 11/26/24
    {
        clientsInRoom.forEach((id, client) -> {
            if(client.getClientName().equals(clientToUnmute))
            {
                if (!sender.isMuted(clientToUnmute))
                    return;
                sender.removeFromMuteList(clientToUnmute);
                client.sendMessage(String.format("<b>%s</b> has unmuted you!", sender.getClientName()));
            }
        });
    }
