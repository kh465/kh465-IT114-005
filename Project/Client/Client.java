...
import Project.Common.RoomResultsPayload; //kh465 11/22/24
...
import java.util.concurrent.CompletableFuture; //kh465 11/22/24
...
public enum Client {
    ...
    private ArrayList<Long> mutedClients = new ArrayList<Long>(); //this doesn't do anything?
                ...
                switch (command) {
                    ...
                    case ROLL: //kh465 11/22/24
                        try
                        {
                            ...
                            {
                                int arg1 = 1;
                                int arg2 = Integer.parseInt(commandValue);
                                sendRoll(arg1, arg2);
                            }
                        }
                        catch (Exception e)
                        {
                            System.out.println(TextFX.colorize("No value detected. Try /roll # for a regular roll, /roll #d# for dice roll", Color.YELLOW));
                        }
                            wasCommand = true;
                            break;
                    case FLIP: //kh465 11/22/24
                        ...
                        wasCommand = true;
                        break;
                    case MUTE: //kh465 11/22/24
                        knownClients.forEach((id, client) -> {
                            if (client.getClientName().equals(commandValue))
                            {
                                try 
                                {
                            ...
                            {
                                LoggerUtil.INSTANCE.warning(String.format("%s does not exist!", commandValue));
                            }
                        });
                        wasCommand = true;
                        break;
    ...
    private void sendMute(String clientToMute) throws IOException//kh465 11/26/24
    {
        DMPayload mute = new DMPayload();
        LoggerUtil.INSTANCE.info(String.format("Captured username: %s", clientToMute));
        mute.setRecClientName(clientToMute);
        mute.setPayloadType(PayloadType.MUTE);
        send(mute);
    }

    private void sendUnmute(String clientToUnmute) throws IOException//kh465 11/26/24
    {
        DMPayload unmute = new DMPayload();
        LoggerUtil.INSTANCE.info(String.format("Captured username: %s", clientToUnmute));
        unmute.setRecClientName(clientToUnmute);
        unmute.setPayloadType(PayloadType.UNMUTE);
        send(unmute);
    }
   
    private void sendRoll(int a1, int a2) throws IOException //kh465 11/22/24
   {
        RollPayload p = new RollPayload(a1, a2);
        p.setPayloadType(PayloadType.ROLL);
        send(p);
   }
   private void sendFlip() throws IOException //kh465 11/22/24
   {
    ...
    public void sendMessage(String message) throws IOException {
        ...
        if (message.startsWith(DM)) //kh465 11/26/24
        {
            sendDM(message);
        }
        else
    ...
    public void sendDM(String message) throws IOException //kh465 11/25/24
    {
        String removeAt = message.replace(DM, "");
        String[] userAndMessage = removeAt.split(" ", 2);
        knownClients.forEach((id, client) -> {
            if (client.getClientName().equals(userAndMessage[0]))
            {
                long recipientClientId = client.getClientId();
                LoggerUtil.INSTANCE.info(String.format("Recipient name and ID: %s %s", userAndMessage[0], recipientClientId));
                DMPayload dm = new DMPayload();
                dm.setRecClientId(recipientClientId);
                dm.setMessage(userAndMessage[1]);
                dm.setPayloadType(PayloadType.DM);
                try {
                    send(dm);
                } catch (IOException ex) {}
            }
            ...
            {
                LoggerUtil.INSTANCE.warning(String.format("%s does not exist!", userAndMessage[0]));
            }
        });
    }
        ...
        try {
            ...
            switch (payload.getPayloadType()) {
                ...
                case PayloadType.MESSAGE: // displays a received message
                    ...
                    break;
                case PayloadType.DM:
                    processMessage(payload.getClientId(), payload.getMessage()); //kh465 11/26/24
                    break;
                case PayloadType.MUTE:
                    DMPayload mute = (DMPayload) payload;
                    mutedClients = mute.getMutedClients();
                    break;
                case PayloadType.UNMUTE:
                    DMPayload unmute = (DMPayload) payload; //i don't think this code does anything?
                    mutedClients = unmute.getMutedClients();
                default:
                    break;
            }
        } catch (Exception e) {
    ...
    private void processMute(long clientId) //kh465 11/28/24
    {
