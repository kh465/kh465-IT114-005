...
import java.io.BufferedWriter; //kh465 12/9/24
import java.io.FileWriter; //kh465 12/9/24
...
public class ChatPanel extends JPanel {
    ...
    public ChatPanel(ICardControls controls) {
        ...
        export.addActionListener((event) -> {
            SwingUtilities.invokeLater(() -> {
                String filePath = String.format("Project/Chatlog %s.txt", LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd_HH-mm-ss")));
                Component[] component = chatArea.getComponents();
                try (BufferedWriter bw = new BufferedWriter(new FileWriter(filePath)))
                {
                    for (Component parts : component)
                    {
                        if (parts instanceof JEditorPane)
                        {
                            StringBuilder sb = new StringBuilder();
                            sb.append(((JEditorPane) parts).getText());
                            bw.write(sb.toString());
                        }
                    }
                    LoggerUtil.INSTANCE.info(String.format("Exporting chatlogs to %s", filePath));
                } catch (IOException e)
                {
                    LoggerUtil.INSTANCE.severe("Something went wrong!");
                    e.printStackTrace();
                }
            });
        });
    ...
    public void exportText() //ignore this, this was when i wasn't sure of what i was doing
    {
        String filePath = String.format("Project/chatlog %s.txt", LocalDateTime.now());
        Component[] parts = chatArea.getComponents();
    ...
    public void addText(String text) {
        SwingUtilities.invokeLater(() -> {
            JEditorPane textContainer = new JEditorPane("text/html", text); //kh465 11/22/24. changed text/plain to text/html
