...
import Project.Common.RollPayload; //kh465 11/13/24
...
public enum Client {
            ...
            if (text.startsWith(COMMAND_CHARACTER)) {
                ...
                switch (command) {
                    ...
                    case ROLL: //kh465 11/13/24
                        ...
                        {
                            System.out.println(TextFX.colorize("No value detected. Try /roll # for a regular roll, /roll #d# for dice.", Color.YELLOW));
                        }
                        wasCommand = true;
                        break;
    ...
    private void sendRoll(int a1, int a2) //kh465 11/13/24
    {
    ...
    private void sendFlip() //kh465 11/13/24
    {
        Payload p = new Payload();
    ...
    // payload processors
    /*private void processRoll(long clientId, int arg1, int arg2)
    {
        ClientPlayer cp = knownClients.get(clientId);
        roll = new Roll(arg1, arg2);
        System.out.println(TextFX.colorize(String.format("%s rolled a %sd%s which resulted in: %s", cp.getClientName(), roll.getArg1(), roll.getArg2(), roll.diceRoll()), Color.CYAN));
