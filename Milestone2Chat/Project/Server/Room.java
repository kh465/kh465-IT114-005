...
import java.util.Random; //kh465 11/13/24
...
public class Room implements AutoCloseable{
    ...
    protected synchronized void sendMessage(ServerThread sender, String message) {
        ...
        if (message.matches("\\*\\*(.*?)\\*\\*") || message.matches("\\*(.*?)\\*") || message.matches("_(.*?)_")) //kh465 11/13/24
        ...
        final String MESSAGE = message; //kh465 11/13/24
        info(String.format("sending message to %s recipients: %s", clientsInRoom.size(), message));
        clientsInRoom.values().removeIf(client -> {
            final String CLIENT_MESSAGE = MESSAGE; //kh465 11/13/24
    ...
    protected void handleRoll(ServerThread sender, int arg1, int arg2) //kh465 11/13/24
    {
        ...
        if (arg1 != 1)
            sendMessage(null, String.format("%s rolled a %sd%s and got %s!", sender.getClientName(), arg1, arg2, roll));
        else
            sendMessage(null, String.format("%s rolled %s and got %s!", sender.getClientName(), arg2, roll));

    }

    protected void handleFlip(ServerThread sender) //kh465 11/13/24
    {
        Random rng = new Random();
        if (rng.nextInt() % 2 == 0)
        {
            sendMessage(null, String.format("%s flipped a coin and got %s!", sender.getClientName(), "heads"));
        }
        else
        sendMessage(null, String.format("%s flipped a coin and got %s!", sender.getClientName(), "tails"));
    }
